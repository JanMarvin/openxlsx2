...
table_ids <- function(wb) {
  ...
  if (!all(identical(unlist(wb$worksheets_rels), character()))) {
    ...
    relship$tid <- cdigit(relship$Target, as_integer = TRUE)
  ...
  public = list(
    ...
    add_chartsheet = function(
      ...
      if (is_waiver(sheet)) {
        ...
        max_sheet_num <- max(
          0,
          cdigit(default_sheets, as_integer = TRUE)
        )
        sheet <- paste0(
          default_sheet_name,
          max_sheet_num + 1L
      ...
      if (length(from$worksheets[[old]]$relships$drawing)) {
        ...
        for (dl in seq_along(drawings_rels)) {
          ...
          for (cf in chartfiles) {
            ...
            old_chart <- cdigit(cf, as_integer = TRUE)
            ...
            if (all(nchar(self$drawings_rels[[new_drawing_rels]]))) {
              ...
              if (ncol(drels) && any(basename(drels$Type) == "image")) {
                ...
                for (i in seq_along(onams)) {
                  media_id   <- cdigit(onams[i], as_integer = TRUE)
    ...
    add_hyperlink = function(
      ...
      if (length(self$worksheets_rels[[sheet]])) {
        ...
        rel_ids  <- cdigit(relships$Id, as_integer = TRUE)
      }
      ...
      if (length(self$worksheets_rels[[sheet]])) {
        relships <- rbindlist(xml_attr(self$worksheets_rels[[sheet]], "Relationship"))
        rel_ids  <- cdigit(relships$Id[basename(relships$Type) == "hyperlink"], as_integer = TRUE)
    ...
    copy_cells = function(
      ...
      start_row <- cdigit(dims, as_integer = TRUE)
    ...
    remove_worksheet = function(sheet = current_sheet()) {
      ...
      if (!is.null(self$worksheets_rels[[sheet]])) {
        ...
        if (nrow(xml_rels) && ncol(xml_rels)) {
          ...
          xml_rels$target_ind <- cdigit(xml_rels$target, as_integer = TRUE)
        }
    ...
    add_drawing = function(
      ...
      if (!is.null(dims) && !is_chartsheet && xml_node_name(xml, "xdr:wsDr") == "xdr:absoluteAnchor") {
        ...
        if (twocell) {
          ...
          rows <- cdigit(dims_list, as_integer = TRUE)
          ...
          cols <- col2int(dims)
          rows <- cdigit(dims, as_integer = TRUE)
      ...
      # the file should still be opened fine with spreadsheet software.
      xml <- xml_attr_mod(
        xml,
      ...
      if (length(self$worksheets_rels[[sheet]]) == 0) {
        ...
        relship$typ <- basename(relship$Type)
        next_relship <- max(cdigit(relship$Id, as_integer = TRUE)) + 1L
    ...
    add_form_control = function(
      ...
      if (length(self$worksheets_rels[[sheet]]) == 0) {
        ...
        relship <- rbindlist(xml_attr(self$worksheets_rels[[sheet]], "Relationship"))
        relship$typ <- basename(relship$Type)
        next_relship <- cdigit(relship$Id, as_integer = TRUE) + 1L
        has_no_drawing <- !any(relship$typ == "drawing")
    ...
    add_border = function(
      ...
      # Ensure if style is NULL, color is NULL and vice versa
      sides <- list(
        top    = list(b = top_border,    c = top_color),
        bottom = list(b = bottom_border, c = bottom_color),
        left   = list(b = left_border,   c = left_color),
        right  = list(b = right_border,  c = right_color)
      )
      ...
      diag_val <- if (length(diag_styles) == 1) diag_styles else NULL
      d_up     <- if (!is.null(diagonal_up)) TRUE else ""
      d_down   <- if (!is.null(diagonal_down)) TRUE else ""
      if (!is.null(diagonal_color)) assert_color(diagonal_color)
      ...
      grid_map <- list(
        top_l    = list(r = 1,  c = 1),
        top_mid  = list(r = 1,  c = cols_mid),
        top_r    = list(r = 1,  c = nc),
        mid_l    = list(r = rows_mid, c = 1),
        core     = list(r = rows_mid, c = cols_mid),
        mid_r    = list(r = rows_mid, c = nc),
        bot_l    = list(r = nr, c = 1),
        bot_mid  = list(r = nr, c = cols_mid),
        bot_r    = list(r = nr, c = nc)
      )

      # Track processed cells to avoid double-styling (e.g., if nr=1, top_l is the same as bot_l)
      processed_cells <- character(0)

      for (tag in names(grid_map)) {
        zone <- grid_map[[tag]]
        z_dims <- as.character(unlist(df[zone$r, zone$c]))

        # Only process if zone exists and hasn't been handled by a previous tag
        if (length(z_dims) > 0 && !any(z_dims %in% processed_cells)) {
          apply_logic(zone$r, zone$c, tag)
          processed_cells <- c(processed_cells, z_dims)
        }
      }
  ...
  private = list(
    ...
    get_drawingsref = function() {
      ...
      for (i in has_drawing) {
        ...
        rblst$id   <- cdigit(rblst$Target, as_integer = TRUE)
        rblst$sheet <- i
