...
wbWorkbook <- R6::R6Class(
  ...
  #' @description Minimal invasive update of cell(s) inside of imported workbooks.
  #'
  ...
  #'    wb$update_cell(x = c(1:3), sheet = "Sheet1", cell = "D4:D6")
  #'
  #'    # update Cells B3:D3 (names())
  #'    wb$update_cell(x = c("x", "y", "z"), sheet = "Sheet1", cell = "B3:D3")
  #'
  #'    # update D4 again (single value this time)
  #'    wb$update_cell(x = 7, sheet = "Sheet1", cell = "D4")
  #'
  #'    # add new column on the left of the existing workbook
  #'    wb$update_cell(x = 7, sheet = "Sheet1", cell = "A4")
  #'
  #'    # add new row on the end of the existing workbook
  #'    wb$update_cell(x = 7, sheet = "Sheet1", cell = "A9")
  #'    wb_to_df(wb)
  #'
  #' @returns The `wbWorkbook` object
  update_cell = function(
    ...
    if (is.character(sheet)) {
      sheet_id <- which(sheet == self$sheet_names)
    } else {
      sheet_id <- sheet
    }
    ...
    cc  <- self$worksheets[[sheet_id]]$sheet_data$cc
    ...
    if (!all(rows %in% rows_in_wb)) {
      ...
      self$worksheets[[sheet_id]]$sheet_data$row_attr <- row_attr_new
      # provide output
    ...
    self$worksheets[[sheet_id]]$dimension <- paste0("<dimension ref=\"", min_cell, ":", max_cell, "\"/>")
        ...
        for (col in cols) {
          ...
          if ((data_class[m] == openxlsx2_celltype[["character"]]) || ((colNames == TRUE) && (n == 1))) {
            ...
            if (cc[sel, "c_s"] == "_openxlsx_NA_")
              cc[sel, "c_s"] <- self$styles_mgr$get_xf_id("hyperlinkstyle")
          } else {
    ...
    self$worksheets[[sheet_id]]$sheet_data$cc  <- cc
  ...
  #' wb$write_data("sheet1", mtcars, colNames = TRUE, rowNames = TRUE)
  #'
  #' wb$add_worksheet("sheet2")
  #' wb$write_data("sheet2", cars, colNames = FALSE)
  #'
  #' wb$add_worksheet("sheet3")
  #' wb$write_data("sheet3", letters)
  #'
  #' wb$add_worksheet("sheet4")
  #' wb$write_data("sheet4", as.data.frame(Titanic), startRow = 2, startCol = 2)
  #'
  #' \dontrun{
  ...
  write_data = function(
    ...
    if (any(dc == openxlsx2_celltype[["hyperlink"]])) {
      if (!length(self$styles_mgr$get_font_id("hyperlinkfont"))) {
        hyperlinkfont <- create_font(
          color = c(rgb = "FF0000FF"),
          name = wb_get_base_font(self)$name$val,
          u = "single")

        self$styles_mgr$add(hyperlinkfont, "hyperlinkfont")

        hyperlink_xf <- create_cell_style(fontId = self$styles_mgr$get_font_id("hyperlinkfont"))
        self$styles_mgr$add(hyperlink_xf, "hyperlinkstyle")
      }
    }
    ...
    hconvert_date1904 <- grepl('date1904="1"|date1904="true"',
                               stri_join(unlist(self$workbook), collapse = ""),
                               ignore.case = TRUE)
    ...
    # this requires access to self$workbook.
    # TODO The check for existing names is in write_data()
    if (!is.null(name)) {

      sheet_name <- self$sheet_names[[sheetno]]
      ...
      self$workbook$definedNames <- c(self$workbook$definedNames, def_name)
    }

    # from here on only self$worksheets is required
    if (is.null(self$worksheets[[sheetno]]$sheet_data$cc)) {

      self$worksheets[[sheetno]]$dimension <- paste0("<dimension ref=\"", dims, "\"/>")
      ...
      self$worksheets[[sheetno]]$sheet_data$row_attr <- rows_attr
      ...
      if (any(dc == openxlsx2_celltype[["numeric"]])) { # numeric or integer
        if (!is.null(unlist(options("openxlsx2.numFmt")))) {
          ...
          self$styles_mgr$add(cust_numFmt, numeric_fmtid)
          numfmt_num <- self$styles_mgr$get_numfmt_id(numeric_fmtid)
          numeric_fmt <- write_xf(nmfmt_df(numfmt_num))
          self$styles_mgr$add(numeric_fmt, numeric_fmtid)
        }
      }
      if (any(dc == openxlsx2_celltype[["short_date"]])) { # Date
        if (is.null(unlist(options("openxlsx2.dateFormat")))) {
          ...
          self$styles_mgr$add(cust_dateFormat, "dateFormat")
          numfmt_dt <- self$styles_mgr$get_numfmt_id("dateFormat")
        }
        short_date_fmt <- write_xf(nmfmt_df(numfmt_dt))
        self$styles_mgr$add(short_date_fmt, short_date_fmtid)
      }
      if (any(dc == openxlsx2_celltype[["long_date"]])) {
        if (is.null(unlist(options("openxlsx2.datetimeFormat")))) {
          ...
          self$styles_mgr$add(cust_datetimeFormat, "datetimeFormat")
          numfmt_posix <- self$styles_mgr$get_numfmt_id("datetimeFormat")
        }
        long_date_fmt  <- write_xf(nmfmt_df(numfmt_posix))
        self$styles_mgr$add(long_date_fmt, long_date_fmtid)
      }
      if (any(dc == openxlsx2_celltype[["accounting"]])) { # accounting
        if (is.null(unlist(options("openxlsx2.accountingFormat")))) {
          ...
          self$styles_mgr$add(cust_accountingFormat, "accounting")
          numfmt_accounting <- self$styles_mgr$get_numfmt_id("accounting")
        }
        accounting_fmt <- write_xf(nmfmt_df(numfmt_accounting))
        self$styles_mgr$add(accounting_fmt, accounting_fmtid)
      }
      if (any(dc == openxlsx2_celltype[["percentage"]])) { # percentage
        if (is.null(unlist(options("openxlsx2.percentageFormat")))) {
          ...
          self$styles_mgr$add(cust_percentageFormat, "percentage")
          numfmt_percentage <- self$styles_mgr$get_numfmt_id("percentage")
        }
        percentage_fmt <- write_xf(nmfmt_df(numfmt_percentage))
        self$styles_mgr$add(percentage_fmt, percentage_fmtid)
      }
      if (any(dc == openxlsx2_celltype[["scientific"]])) {
        if (is.null(unlist(options("openxlsx2.scientificFormat")))) {
          ...
          self$styles_mgr$add(cust_scientificFormat, "scientific")
          numfmt_scientific <- self$styles_mgr$get_numfmt_id("scientific")
        }
        scientific_fmt <- write_xf(nmfmt_df(numfmt_scientific))
        self$styles_mgr$add(scientific_fmt, scientific_fmtid)
      }
      if (any(dc == openxlsx2_celltype[["comma"]])) {
        if (is.null(unlist(options("openxlsx2.comma")))) {
          ...
          self$styles_mgr$add(cust_scientificFormat, "comma")
          numfmt_comma <- self$styles_mgr$get_numfmt_id("comma")
        }
        comma_fmt <- write_xf(nmfmt_df(numfmt_comma))
        self$styles_mgr$add(comma_fmt, comma_fmtid)
      }
      ...
      cc$c_s[cc$typ == "0"]  <- self$styles_mgr$get_xf_id(short_date_fmtid)
      cc$c_s[cc$typ == "1"]  <- self$styles_mgr$get_xf_id(long_date_fmtid)
      if (length(self$styles_mgr$get_xf_id(numeric_fmtid)) == 1) {
        cc$c_s[cc$typ == "2"]  <- self$styles_mgr$get_xf_id(numeric_fmtid)
      }
      cc$c_s[cc$typ == "6"]  <- self$styles_mgr$get_xf_id(accounting_fmtid)
      cc$c_s[cc$typ == "7"]  <- self$styles_mgr$get_xf_id(percentage_fmtid)
      cc$c_s[cc$typ == "8"]  <- self$styles_mgr$get_xf_id(scientific_fmtid)
      cc$c_s[cc$typ == "9"]  <- self$styles_mgr$get_xf_id(comma_fmtid)
      cc$c_s[cc$typ == "10"] <- self$styles_mgr$get_xf_id("hyperlinkstyle")

      self$worksheets[[sheetno]]$sheet_data$cc <- cc
