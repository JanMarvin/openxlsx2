...
#' helper to update/create extLst on worksheet
#'
#' newer spreadsheet functions live in the extLst also called future record table.
#' if we update this table, we have to make sure that (1) we do not lose any already available entry such as slicers or timelines
#' and (2) that we assign the newest entry to the correct version. an x14 namespace can only hold x14 children. mixing x14 and x15 can cause broken files
#' @param extLst extLst
#' @param newExtLst newExtLst
#' @keywords internal
#' @noRd
update_extLst <- function(extLst, newExtLst) {
  ...
  if (length(extLst) == 0) {
    ...
    # extLst might be x15
    if (!any(grepl("x14:conditionalFormattings", extLst))) {
      extLst <- c(
        extLst,
        sprintf('<ext uri="{78C0D931-6437-407d-A8EE-F0AAD7539E65}" xmlns:x14="http://schemas.microsoft.com/office/spreadsheetml/2009/9/main"><x14:conditionalFormattings/></ext>')
      )
    }

    is_ext_x14 <- grepl("x14:conditionalFormattings", extLst)

    # extLst is available, has conditionalFormattings
    extLst[is_ext_x14] <- xml_add_child(
      extLst[is_ext_x14],
      xml_node(
        newExtLst,
        "ext",
