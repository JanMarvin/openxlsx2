...
# TODO use wb$wb_set_col_widths()
  ...
  expect_silent(wb_set_col_widths(wb, "test", widths = 12L, cols = seq_along(mtcars)))
  expect_equal(
  ...
  # wrong sheet
  expect_error(wb_set_col_widths(wb, "test2", widths = 12L, cols = seq_along(mtcars)))

  # reset the column with, we do not provide an option ot remove the column entry
  expect_silent(wb_set_col_widths(wb, "test", cols = seq_along(mtcars)))
  expect_equal(
  ...
  expect_silent(wb_set_col_widths(wb, "test", cols = "Y", widths = 22))
  expect_equal(
