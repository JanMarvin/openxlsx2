...
test_that("createComment() works", {
  # error checking
  expect_silent(createComment("hi", width = 1))
  expect_error(createComment("hi", width = 1L))
  expect_error(createComment("hi", width = 1:2))

  expect_silent(createComment("hi", height = 1))
  expect_error(createComment("hi", height = 1L))
  expect_error(createComment("hi", height = 1:2))

  expect_error(createComment("hi", visible = NULL))
  expect_error(createComment("hi", visible = c(TRUE, FALSE)))

  expect_error(createComment("hi", author = 1))
  expect_error(createComment("hi", author = c("a", "a")))

  expect_true(inherits(createComment("Hello"), "wbComment"))
})
...
test_that("comments", {
  ...
  expect_silent(writeComment(wb, 1, col = 6, row = 3, comment = c3))

  expect_true(length(wb$comments) == 1)
  expect_true(length(wb$comments[[1]]) == 3)

  expect_silent(removeComment(wb, 1, col = "B", row = 10))
