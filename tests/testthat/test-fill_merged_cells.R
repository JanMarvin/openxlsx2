test_that("fill merged cells", {
  ...
  expect_equal(read_xlsx(tmp_file, dims = wb_dims(cols = 1:3, rows = 1:4), fill_merged_cells = TRUE), r2_1[1:3, ], ignore_attr = TRUE)
})

test_that("merge and unmerge cells", {

  dims <- wb_dims(rows = 1:2, cols = 1:2)
  wb <- wb_workbook()$add_worksheet()$merge_cells(dims = dims)

  expect_error(wb$merge_cells(dims = dims), "Remove existing merge first.")
  expect_silent(wb$unmerge_cells(dims = dims))
  expect_silent(wb$merge_cells(dims = dims))
