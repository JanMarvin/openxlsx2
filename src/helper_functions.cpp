...
static SEXP sym_tzone = Rf_install("tzone");
static SEXP sym_units = Rf_install("units");
...
Rcpp::NumericVector as_numeric(Rcpp::Nullable<Rcpp::RObject> input) {
  ...
  case REALSXP: {
    ...
    if (Rf_inherits(obj, "Date") || Rf_inherits(obj, "POSIXct") || Rf_inherits(obj, "difftime")) {
      ...
      Rf_setAttrib(out, R_ClassSymbol, R_NilValue);
      Rf_setAttrib(out, sym_tzone, R_NilValue);
      Rf_setAttrib(out, sym_units, R_NilValue);
      return out;
    }
