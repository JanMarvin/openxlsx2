...
static uint64_t Rot(uint64_t x, uint64_t n)        { return (x >> (n & 63)) | (x << (64 - (n & 63))); }
static uint64_t Sh(uint64_t x, uint64_t n)         { return x >> n; }
static uint64_t Sigma0(uint64_t x)            { return Rot(x, 28) ^ Rot(x, 34) ^ Rot(x, 39); }
static uint64_t Sigma1(uint64_t x)            { return Rot(x, 14) ^ Rot(x, 18) ^ Rot(x, 41); }
static uint64_t Gamma0(uint64_t x)            { return Rot(x, 1) ^ Rot(x, 8) ^ Sh(x, 7); }
static uint64_t Gamma1(uint64_t x)            { return Rot(x, 19) ^ Rot(x, 61) ^ Sh(x, 6); }
...
static void sha_compress(sha512_state& md, const unsigned char *buf)
{
    ...
    // Compress
    auto RND = [&](uint64_t a, uint64_t b, uint64_t c, uint64_t& d, uint64_t e, uint64_t f, uint64_t g, uint64_t& h, uint64_t i)
    {
...
void sha_process(sha512_state& md, const void* src, uint32_t inlen)
{
    const uint32_t block_size = sizeof(sha512_state::buf);
