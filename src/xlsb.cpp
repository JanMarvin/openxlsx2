    ...
    while (!end_of_style_sheet) {
      ...
      switch (x) {
        ...
        case BrtFmt: {
          ...
          out << " numFmtId=\"" << ifmt << "\" formatCode=\""
              << escape_xml(stFmtCode);
          out << "\" />" << std::endl;
          break;
        }
        ...
        case BrtFont: {
          out << "<font>" << std::endl;
          uint8_t uls = 0, bFamily = 0, bCharSet = 0, unused = 0,
                  bFontScheme = 0;
          ...
          if (fields->fItalic)
            out << "<i/>" << std::endl;
          if (fields->fStrikeout)
            out << "<strike/>" << std::endl;
          // if (fields->fOutline) out << "<outline/>" << std::endl;

          if (uls > 0) {
            if (uls == 0x01)
              out << "<u val=\"single\" />" << std::endl;
            if (uls == 0x02)
              out << "<u val=\"double\" />" << std::endl;
            if (uls == 0x21)
              out << "<u val=\"singleAccounting\" />" << std::endl;
            if (uls == 0x22)
              out << "<u val=\"doubleAccounting\" />" << std::endl;
          }

          out << "<sz val=\"" << dyHeight << "\" />" << std::endl;

          // if (color[0] == 0x01) { // wrong but right?
          //   Rcpp::Rcout << "<color theme=\"" << color[1] << "\" />" <<
          //   std::endl;
          // }
          ...
          if (color[0] == 0x02) {
            out << "<color rgb=\""
                << to_argb(color[6], color[3], color[4], color[5]) << "\" />"
                << std::endl;
          }
          ...
          if (bFamily > 0)
            out << "<family val=\"" << (uint16_t)bFamily << "\" />"
                << std::endl;
          ...
          if (bCharSet > 0)
            out << "<charset val=\"" << (uint16_t)bCharSet << "\" />"
                << std::endl;
        ...
        case BrtFill: {
          ...
          double xnumDegree = 0.0, xnumFillToLeft = 0.0, xnumFillToRight = 0.0,
                 xnumFillToTop = 0.0, xnumFillToBottom = 0.0,
                 aPos = 0.0  // not used
              ;
          ...
          if (debug) {
            Rcpp::Rcout << "unused gradinet filll colors" << xnumDegree << ": "
                        << xnumFillToLeft << ": " << ": " << xnumFillToRight
                        << ": " << xnumFillToTop << "; " << xnumFillToBottom
                        << std::endl;
          }
          ...
          for (uint32_t i = 0; i < cNumStop; ++i) {
            ...
            if (debug) {
              Rcpp::Rcout << "unused gradient Colors: "
                          << to_argb(fgColor[6], fgColor[3], fgColor[4],
                                     fgColor[5])
                          << ": " << aPos << std::endl;
            }
          }
          ...
          if (fls != 0x00000028) {
            ...
            if (bgColor[2] != 0)
              bgtint = (double)bgColor[2] / 32767;
            if (fgColor[2] != 0)
              fgtint = (double)fgColor[2] / 32767;
            ...
            if (fgColor[0] == 0x02)
              out << "<fgColor rgb=\""
                  ...
                  << "\" />";
            if (fgColor[0] == 0x03)
              out << "<fgColor theme=\"" << fgColor[1] << "\" tint=\""
                  << fgstream.str() << "\" />";
            ...
            if (bgColor[0] == 0x02)
              out << "<bgColor rgb=\""
                  << to_argb(bgColor[6], bgColor[3], bgColor[4], bgColor[5])
                  << "\" />";
            if (bgColor[0] == 0x03)
              out << "<bgColor theme=\"" << bgColor[1] << "\" tint=\""
                  << fgstream.str() << "\" />";
        ...
        case BrtXF: {
          ...
          if (fields->alc > 0 || fields->alcv > 0 || indent || trot ||
              fields->iReadingOrder || fields->fShrinkToFit || fields->fWrap ||
              fields->fJustLast) {
            out << "><alignment";
            ...
            // I have a file which is left to right, but this here returns 2:
            // right to left
        ...
        case BrtIndexedColor: {
          ...
          // if (color[0] == 0x02) {
          out << "<rgbColor rgb=\"" << to_argb(reserved, bRed, bGreen, bRed)
              << std::dec << "\" />" << std::endl;
          // }
          break;
        }
        ...
        case BrtBeginDXFs: {
          ...
          // This is a todo. the xfPropDataBlob is almost the entire styles part
          // again case BrtDXF:
          // {
    ...
    while (!end_of_table) {
      ...
      switch (x) {
        ...
        case BrtBeginList: {
          ...
          std::string ref =
              int_to_col(rfxList[2] + 1) + std::to_string(rfxList[0] + 1) +
              ":" + int_to_col(rfxList[3] + 1) + std::to_string(rfxList[1] + 1);
          ...
          uint32_t nDxfHeader = 0, nDxfData = 0, nDxfAgg = 0, nDxfBorder = 0,
                   nDxfHeaderBorder = 0, nDxfAggBorder = 0, dwConnID = 0;
          ...
          out << "<table "
                 ...
                 "xmlns:xr=\"http://schemas.microsoft.com/office/spreadsheetml/"
                 "2014/revision\" "
                 "xmlns:xr3=\"http://schemas.microsoft.com/office/"
                 "spreadsheetml/2016/revision3\" mc:Ignorable=\"xr xr3\""
              << " id=\"" << idList << "\" name=\"" << stName
              << "\" displayName=\"" << stDisplayName << "\" ref=\"" << ref
              << "\" totalsRowShown=\"" << crwTotals << "\">" << std::endl;
        ...
        case BrtBeginAFilter: {
          if (debug)
            Rcpp::Rcout << "<autofilter>" << std::endl;
          std::vector<int32_t> rfx = UncheckedRfX(bin, swapit);

          std::string lref =
              int_to_col(rfx[2] + 1) + std::to_string(rfx[0] + 1);
          std::string rref =
              int_to_col(rfx[3] + 1) + std::to_string(rfx[1] + 1);
        ...
        case BrtBeginFilterColumn: {
          if (debug)
            Rcpp::Rcout << "<filterColumn>" << std::endl;
        ...
        case BrtBeginFilters: {
          if (debug)
            Rcpp::Rcout << "<filters>" << std::endl;
          // bin.seekg(size, bin.cur);

          uint32_t fBlank = 0, unused = 0;
          fBlank = readbin(fBlank, bin,
                           swapit);  // a 32bit flag, after all ... why not?
        ...
        case BrtColorFilter: {
          ...
          out << "<colorFilter dxfId=\"" << dxfid << "\" cellColor=\""
              << fCellColor << "\"/>" << std::endl;
        ...
        case BrtIconFilter14: {
          ...
          std::string iconSet;
          if (iIconSet)
            iconSet = to_iconset(iIconSet);

          out << "<iconFilter iconSet=\"" << iconSet << "\" iconId=\"" << iIcon
              << "\"/>" << std::endl;
        ...
        case BrtBeginListCol: {
          if (debug)
            Rcpp::Rcout << "<tableColumn>" << std::endl;
          // bin.seekg(size, bin.cur);
          // break;

          uint32_t idField = 0, ilta = 0, nDxfHdr = 0, nDxfInsertRow = 0,
                   nDxfAgg = 0, idqsif = 0;
          ...
          if (debug)
            Rcpp::Rcout << stName << ": " << stCaption << ": " << stTotal
                        << std::endl;
          std::string stStyleHeader = XLNullableWideString(bin, swapit);
          std::string stStyleInsertRow = XLNullableWideString(bin, swapit);
          std::string stStyleAgg = XLNullableWideString(bin, swapit);
          if (debug)
            Rcpp::Rcout << stStyleHeader << ": " << stStyleInsertRow << ": "
                        << stStyleAgg << std::endl;

          out << "<tableColumn id=\"" << idField << "\" name=\"" << stCaption
              << "\">" << std::endl;
        ...
        case BrtListCCFmla: {
          ...
          if (debug)
            Rcpp::warning("Table formulas are not fully implemented.");
        ...
        case BrtRRDefName: {
          ...
          // -- have not seen this yet. if it appears, treat it as if a revision
          // record was found -- rgce.rgce or rgceOld.rgce in BrtRRDefName
          if (debug)
            Rcpp::Rcout << "BrtRRChgCell or BrtRRDefName" << std::endl;
        ...
        default: {
          if (debug) {
            Rcpp::Rcout << std::to_string(x) << ": " << std::to_string(size)
                        << " @ " << bin.tellg() << std::endl;
          }
          bin.seekg(size, bin.cur);
          break;
    ...
    while (!end_of_comments) {
      ...
      switch (x) {
        ...
        case BrtCommentText: {
          ...
          // out << "<text><r><rPr><sz val=\"10\"/><color
          // rgb=\"FF000000\"/><rFont val=\"Tahoma\"/><family
          // val=\"2\"/></rPr><t>" << escape_xml(commentText) <<
          // "</t></r></text>" << std::endl;
...
// [[Rcpp::export]]
int32_t externalreferences_bin(std::string filePath,
                               std::string outPath,
                               bool debug) {
  std::ofstream out(outPath);
  std::ifstream bin(filePath, std::ios::in | std::ios::binary | std::ios::ate);
      ...
      switch (x) {
        case BrtBeginSupBook: {
          ...
          out << "<externalLink "
                 ...
                 "xmlns:xxl21=\"http://schemas.microsoft.com/office/"
                 "spreadsheetml/2021/extlinks2021\" mc:Ignorable=\"x14 xxl21\">"
              << std::endl;

          // assume for now all external references are workbooks
          out << "<externalBook "
                 "xmlns:r=\"http://schemas.openxmlformats.org/officeDocument/"
                 "2006/relationships\" r:id=\""
              << string1 << "\">" << std::endl;
        ...
        case BrtExternTableStart: {
          if (debug)
            Rcpp::Rcout << "<BrtExternTableStart>" << std::endl;
          uint8_t flags = 0;
          uint32_t iTab = 0;
        ...
        case BrtExternRowHdr: {
          if (debug)
            Rcpp::Rcout << "<BrtExternRowHdr>" << std::endl;
        ...
        case BrtExternCellBlank: {
          if (debug)
            Rcpp::Rcout << "<BrtExternCellBlank>" << std::endl;

          int32_t col = UncheckedCol(bin, swapit);
          out << "<cell r=\"" << int_to_col(col + 1) << row + 1 << "\" />"
              << std::endl;
        ...
        case BrtExternCellBool: {
          if (debug)
            Rcpp::Rcout << "<BrtExternCellBool>" << std::endl;

          uint8_t value = 0;
          int32_t col = UncheckedCol(bin, swapit);
          value = readbin(value, bin, swapit);
          out << "<cell r=\"" << int_to_col(col + 1) << row + 1 << "\" t=\"b\">"
              << std::endl;
        ...
        case BrtExternCellError: {
          if (debug)
            Rcpp::Rcout << "<BrtExternCellError>" << std::endl;

          std::string value;
          int32_t col = UncheckedCol(bin, swapit);
          value = BErr(bin, swapit);
          out << "<cell r=\"" << int_to_col(col + 1) << row + 1 << "\" t=\"e\">"
              << std::endl;
        ...
        case BrtExternCellReal: {
          if (debug)
            Rcpp::Rcout << "<BrtExternCellReal>" << std::endl;
        ...
        case BrtExternCellString: {
          if (debug)
            Rcpp::Rcout << "<BrtExternCellString>" << std::endl;
        ...
        case BrtExternTableEnd: {
          if (debug)
            Rcpp::Rcout << "<BrtExternTableEnd>" << std::endl;
...
// [[Rcpp::export]]
int32_t sharedstrings_bin(std::string filePath,
                          std::string outPath,
                          bool debug) {
  std::ofstream out(outPath);
  std::ifstream bin(filePath, std::ios::in | std::ios::binary | std::ios::ate);
    ...
    while (!end_of_shared_strings) {
      ...
      switch (x) {
        case BrtBeginSst: {
          ...
          out << "<sst " << "count=\"" << count << "\" uniqueCount=\""
              << uniqueCount << "\">" << std::endl;
          break;
        }
        ...
        case BrtSSTItem: {
          ...
          if (bin.tellg() < pos) {
            ...
            Rcpp::Rcout << "BrtSSTItem skipping ahead (bytes): " << missing
                        << std::endl;
            // }
...
int32_t workbook_bin(std::string filePath, std::string outPath, bool debug) {
  ...
  if (bin) {
    ...
    while (!end_of_workbook) {
      ...
      switch (x) {
        case BrtBeginBook: {
          ...
          out << "<workbook "
                 ...
                 "xmlns:xr=\"http://schemas.microsoft.com/office/spreadsheetml/"
                 "2014/revision\" "
                 "xmlns:xr6=\"http://schemas.microsoft.com/office/"
                 "spreadsheetml/2016/revision6\" "
                 "xmlns:xr10=\"http://schemas.microsoft.com/office/"
                 "spreadsheetml/2016/revision10\" "
                 "xmlns:xr2=\"http://schemas.microsoft.com/office/"
                 "spreadsheetml/2015/revision2\" mc:Ignorable=\"x15 xr xr6 "
                 "xr10 xr2\">"
              << std::endl;
          bin.seekg(size, bin.cur);
          break;
        ...
        case BrtWbProp: {
          ...
          out << "<workbookPr" <<
              ...
              << " codeName=\"" << strName << "\" " << " date1904=\""
              << fields->f1904 << "\" " << " defaultThemeVersion=\""
              << dwThemeVersion << "\" " << " filterPrivacy=\""
              << fields->fFilterPrivacy << "\" " << " hidePivotFieldList=\""
              << fields->fHidePivotTableFList << "\" "
              ...
              << "\" " << " updateLinks=\""
              << (uint32_t)fields->grbitUpdateLinks << "\" " << "/>"
              << std::endl;
        ...
        case BrtBookView: {
          ...
          uint8_t flags = 0;
          int32_t xWn = 0, yWn = 0;
          uint32_t dxWn = 0, dyWn = 0, iTabRatio = 0, itabFirst = 0,
                   itabCur = 0;
        ...
        case BrtUserBookView: {
          if (debug)
            Rcpp::Rcout << "<customWorkbookView>" << std::endl;

          std::ostringstream cwv;

          int32_t xLeft = 0, xRight = 0, yTop = 0, yBot = 0, iTabid = 0,
                  iTabRatio = 0, guid0 = 0, guid1 = 0, guid2 = 0, guid3 = 0,
                  flags = 0;
          ...
          if (fields->mdDspNote == 0)
            showComments = "commNone";
          if (fields->mdDspNote == 1)
            showComments = "commIndAndComment";
          if (fields->mdDspNote == 2)
            showComments = "commIndicator";

          std::string showObjects;
          if (fields->mdHideObj == 0)
            showObjects = "all";
          if (fields->mdHideObj == 1)
            showObjects = "placeholders";
          if (fields->mdHideObj == 2)
            showObjects = "none";

          cwv << "<customWorkbookView" << std::endl;
          cwv << " name=\"" << stName << "\"" << std::endl;
          cwv << " guid=\"{" << guid_str(guids) << "}\"" << std::endl;
          if (fields->fTimedUpdate)
            cwv << " autoUpdate=\"" << fields->fTimedUpdate << "\""
                << std::endl;
          if (fields->fTimedUpdate)
            cwv << " mergeInterval=\"" << wMergeInterval << "\"" << std::endl;
          if (fields->fAllMemChanges)
            cwv << " changesSavedWin=\"" << fields->fAllMemChanges << "\""
                << std::endl;
          if (fields->fOnlySync)
            cwv << " onlySync=\"" << fields->fOnlySync << "\"" << std::endl;
          if (fields->fPersonalView)
            cwv << " personalView=\"" << fields->fPersonalView << "\""
                << std::endl;
          if (!fields->fPrintIncl)
            cwv << " includePrintSettings=\"" << fields->fPrintIncl << "\""
                << std::endl;
          // wrong?
          // if (!fields->fRowColIncl) cwv << " includeHiddenRowCol=\"" <<
          // fields->fRowColIncl << "\"" << std::endl;
          if (fields->fZoom)
            cwv << " maximized=\"" << fields->fZoom << "\"" << std::endl;
          if (fields->fIconic)
            cwv << " minimized=\"" << fields->fIconic << "\"" << std::endl;
          if (!fields->fDspHScroll)
            cwv << " showHorizontalScroll=\"" << fields->fDspHScroll << "\""
                << std::endl;
          if (!fields->fDspVScroll)
            cwv << " showVerticalScroll=\"" << fields->fDspVScroll << "\""
                << std::endl;
          if (!fields->fBotAdornment)
            cwv << " showSheetTabs=\"" << fields->fBotAdornment << "\""
                << std::endl;
          if (xLeft > 0)
            cwv << " xWindow=\"" << xLeft << "\"" << std::endl;
          if (yTop > 0)
            cwv << " yWindow=\"" << yTop << "\"" << std::endl;
          if (xRight > 0)
            cwv << " windowWidth=\"" << xRight << "\"" << std::endl;
          if ((yBot - yTop) > 0)
            cwv << " windowHeight=\"" << (yBot - yTop) << "\"" << std::endl;
          if (iTabRatio != 600)
            cwv << " tabRatio=\"" << iTabRatio << "\"" << std::endl;
          cwv << " activeSheetId=\"" << iTabid << "\"" << std::endl;
          if (!fields->fDspFmlaBar)
            cwv << " showFormulaBar=\"" << fields->fDspFmlaBar << "\""
                << std::endl;
          if (!fields->fDspStatus)
            cwv << " showStatusbar=\"" << fields->fDspStatus << "\""
                << std::endl;
          if (showComments != "commIndicator")
            cwv << " showComments=\"" << showComments << "\"" << std::endl;
          if (showObjects != "all")
            cwv << " showObjects=\"" << showObjects << "\"" << std::endl;
          cwv << "/>" << std::endl;

          // Rcpp::Rcout << xLeft << ": " << xRight << ": " << yTop << ": " <<
          // yBot << std::endl;
        ...
        case BrtBundleSh: {
          ...
          if (debug)
            Rcpp::Rcout << "sheet vis: " << hsState << ": " << iTabID << ": "
                        << rid << std::endl;
          ...
          out << "<sheet r:id=\"" << rid << "\" state=\"" << visible
              << "\" sheetId=\"" << iTabID << "\" name=\"" << val << "\"/>"
              << std::endl;
          break;
        }
        ...
        case BrtName: {
          ...
          if (debug)
            Rcpp::Rcout << "BrtName endpos: " << end_pos << std::endl;
          ...
          if (debug)
            Rcpp::Rcout << "definedName: " << name << ": " << comment
                        << std::endl;
        ...
        case BrtPlaceholderName: {
          ...
          std::string name = XLWideString(bin, swapit);
          // Rcpp::Rcout << "<internalReference name=\""<< name << "\"/>" <<
          // std::endl;
          break;
        }
        ...
        case BrtSupBookSrc: {
          ...
          out << "<externalReference r:id=\"" << strRelID << "\"/>"
              << std::endl;
        ...
        case BrtExternSheet: {
          if (debug)
            Rcpp::Rcout << "<BrtExternSheet>" << std::endl;
          uint32_t cXti = 0;

          cXti = readbin(cXti, bin, swapit);
          if (cXti > 65535)
            Rcpp::stop("cXti to large");

          std::vector<uint32_t> rgXti(cXti);
          // Rcpp::Rcout << "reference_type: " << reference_type.size() <<
          // std::endl; Rcpp::Rcout << "reference_type: " << cXti << std::endl;
          for (uint32_t i = 0; i < cXti; ++i) {
            std::vector<int32_t> xti = Xti(bin, swapit);
            if ((size_t)xti[0] > reference_type.size())
              Rcpp::stop("references do not match");
            std::string tmp =
                "<xti id=\"" + std::to_string(xti[0]) + "\" firstSheet=\"" +
                std::to_string(xti[1]) + "\" lastSheet=\"" +
                std::to_string(xti[2]) + "\" type=\"" +
                reference_type[static_cast<size_t>(xti[0])] + "\" />";
            xtis.push_back(tmp);
          }
        ...
        case BrtWorkBookPr15: {
          ...
          // fChartTrackingRefBased = readbin(fChartTrackingRefBased, bin, 0) &
          // 0x01;
          break;
        }
      ...
      if (debug)
        Rcpp::Rcout << "wb-loop: " << x << ": " << size << ": " << bin.tellg()
                    << std::endl;
    }
...
// [[Rcpp::export]]
int32_t worksheet_bin(std::string filePath,
                      bool chartsheet,
                      std::string outPath,
                      bool debug) {
  std::ofstream out(outPath);
    ...
    while (!end_of_worksheet) {
      ...
      switch (x) {
        case BrtBeginSheet: {
          ...
          if (debug)
            Rcpp::Rcout << "Begin of <worksheet>: " << bin.tellg() << std::endl;
          break;
        }

        case BrtCsProp: {
          if (debug)
            Rcpp::Rcout << "CsProp: " << bin.tellg() << std::endl;
          ...
          if (color[0] >= 1 && color[0] <= 3) {
            ...
            if (color[0] == 0x03) {
              out << "<tabColor theme=\"" << color[1] << "\" />"
                  << std::endl;  //  << "\" tint=\""<< color[2]
            }
        ...
        case BrtWsDim: {
          ...
          std::string lref =
              int_to_col(dims[2] + 1) + std::to_string(dims[0] + 1);
          std::string rref =
              int_to_col(dims[3] + 1) + std::to_string(dims[1] + 1);
        ...
        case BrtPane: {
          ...
          std::string topLeft =
              int_to_col(colLeft + 1) + std::to_string(rwTop + 1);

          std::string pnn;
          if (pnnAct == 0x00000000)
            pnn = "bottomRight";
          if (pnnAct == 0x00000001)
            pnn = "topRight";
          if (pnnAct == 0x00000002)
            pnn = "bottomLeft";
          if (pnnAct == 0x00000003)
            pnn = "topLeft";
        ...
        case BrtBeginWsView: {
          ...
          uint16_t flags = 0, reserved3 = 0, wScale = 0, wScaleNormal = 0,
                   wScaleSLV = 0, wScalePLV = 0;
          ...
          if (fields->fRightToLeft) {
            Rcpp::Rcout << "rightToLeft=\"" << fields->fRightToLeft << "\""
                        << std::endl;
          ...
          if (colLeft > 0 || rwTop > 0)
            out << " topLeftCell=\"" << int_to_col(colLeft + 1)
                << std::to_string(rwTop + 1) << "\"";
          if (xlView)
        ...
        case BrtPageSetup: {
          if (debug)
            Rcpp::Rcout << "BrtPageSetup: " << std::endl;
          uint16_t flags = 0;
          uint32_t iPaperSize = 0, iScale = 0, iRes = 0, iVRes = 0, iCopies = 0,
                   iPageStart = 0, iFitWidth = 0, iFitHeight = 0;
          ...
          out << "<pageSetup" <<
              ...
              " copies=\"" << iCopies <<
              // "\" errors=\""<<  <<
              "\" firstPageNumber=\"" << iPageStart << "\" fitToHeight=\""
              << iFitHeight << "\" fitToWidth=\"" << iFitWidth
              << "\" horizontalDpi=\"" << iRes <<
              // "\" orientation=\""<<  <<
              // "\" pageOrder=\""<<  <<
              // "\" paperHeight=\""<<  <<
        ...
        case BrtPhoneticInfo: {
          if (debug)
            Rcpp::Rcout << "BrtPhoneticInfo: " << std::endl;
          uint16_t iFnt = 0;
        ...
        case BrtBeginHeaderFooter: {
          if (debug)
            Rcpp::Rcout << "BrtBeginHeaderFooter: " << std::endl;
          uint16_t flags = 0;
          ...
          BrtBeginHeaderFooterFields* fields =
              (BrtBeginHeaderFooterFields*)&flags;

          if (debug)
            Rcpp::Rcout << stHeader << ": " << stFooter << ": " << stHeaderEven
                        << ": " << stFooterEven << ": " << stHeaderFirst << ": "
                        << stFooterFirst << std::endl;

          out << "<headerFooter" << std::endl;
          if (fields->fHFDiffOddEven)
            out << " differentOddEven=\"" << fields->fHFDiffOddEven << "\""
                << std::endl;
          if (fields->fHFDiffFirst)
            out << " differentFirst=\"" << fields->fHFDiffFirst << "\""
                << std::endl;
          if (fields->fHFScaleWithDoc)
            out << " scaleWithDoc=\"" << fields->fHFScaleWithDoc << "\""
                << std::endl;
          if (fields->fHFAlignMargins)
            out << " alignWithMargins=\"" << fields->fHFAlignMargins << "\""
                << std::endl;
          out << ">" << std::endl;

          if (!stHeader.empty())
            out << "<oddHeader>" << stHeader << "</oddHeader>" << std::endl;
          if (!stFooter.empty())
            out << "<oddFooter>" << stFooter << "</oddFooter>" << std::endl;
          if (!stHeaderEven.empty())
            out << "<evenHeader>" << stHeaderEven << "</evenHeader>"
                << std::endl;
          if (!stFooterEven.empty())
            out << "<evenFooter>" << stFooterEven << "</evenFooter>"
                << std::endl;
          if (!stHeaderFirst.empty())
            out << "<firstHeader>" << stHeaderFirst << "</firstHeader>"
                << std::endl;
          if (!stFooterFirst.empty())
            out << "<firstFooter>" << stFooterFirst << "</firstFooter>"
                << std::endl;
          out << "</headerFooter>" << std::endl;
        ...
        case BrtSel: {
          ...
          std::string lref =
              int_to_col(sqrfx[3] + 1) + std::to_string(sqrfx[1] + 1);
          std::string rref =
              int_to_col(sqrfx[4] + 1) + std::to_string(sqrfx[2] + 1);
          ...
          out << "<selection pane=\"" << spnn << "\" activeCell=\"" << ac
              << "\" sqref=\"" << sqref << "\" />" << std::endl;
        ...
        case BrtBeginColInfos: {
          if (debug)
            Rcpp::Rcout << "<cols>: " << bin.tellg() << std::endl;
          out << "<cols>" << std::endl;
          bin.seekg(size, bin.cur);
          break;
        }
        ...
        case BrtColInfo: {
          if (debug)
            Rcpp::Rcout << "<col/>: " << bin.tellg() << std::endl;
          uint16_t colinfo = 0;
        ...
        case BrtRowHdr: {
          if (debug)
            Rcpp::Rcout << "<row/>: " << bin.tellg() << std::endl;
          ...
          if (!first_row) {
            for (size_t i = 0; i < colvec.size(); ++i) {
              out << "<c r=\"" << colvec[i].c_r << "\" s=\"" << colvec[i].c_s
                  << "\" t=\"" << colvec[i].c_t << "\">" << std::endl;
              out << "<v>" << escape_xml(colvec[i].v) << "</v>" << std::endl;
              out << "<f ref=\"" << colvec[i].f_ref << "\" ca=\""
                  << colvec[i].f_ca << "\" si=\"" << colvec[i].f_si << "\" t=\""
                  << colvec[i].f_t << "\" >" << colvec[i].f << "</f>"
                  << std::endl;
              out << "</c>" << std::endl;
            }
          ...
          uint8_t bits3 = 0, fExtraAsc = 0, fExtraDsc = 0, fCollapsed = 0,
                  fDyZero = 0, fUnsynced = 0, fGhostDirty = 0, fReserved = 0,
                  fPhShow = 0;
          uint16_t miyRw = 0;
          ...
          if (ccolspan > 16)
            Rcpp::stop("ccolspan to large");
          std::string spans;

          std::vector<int32_t> spans_int;
          if (ccolspan) {
            // has number of rgBrtColspan elements. For now take max and min
            // value. there are other types of spans (1:1 1024:16384), but
            // unsure how this is supposed to be handled. It's possible to have
            // elements like spans = "1:1 16384:16384" which we will read as
            // spans = "1:16384"
            for (uint8_t clpn = 0; clpn < ccolspan; ++clpn) {
              ...
              if (debug) {
                spans = std::to_string(colMic + 1) + ":" +
                        std::to_string(colLast + 1);
            ...
            int32_t min_span =
                *std::min_element(spans_int.begin(), spans_int.end());
            int32_t max_span =
                *std::max_element(spans_int.begin(), spans_int.end());

            spans = std::to_string(min_span + 1) + ":" +
                    std::to_string(max_span + 1);
          }
          ...
          if (debug)
            Rcpp::Rcout << "ROW:" << (rw + 1) << " : " << ixfe << " : " << miyRw
                        << " : " << (int32_t)fExtraAsc << " : "
                        << (int32_t)fExtraDsc << " : " << unk32 << " : "
                        << (int32_t)fCollapsed << " : " << (int32_t)fDyZero
                        << " : " << (int32_t)fUnsynced << " : "
                        << (int32_t)fGhostDirty << " : " << (int32_t)fReserved
                        << " : " << (int32_t)fPhShow << " : " << ccolspan
                        << "; " << bin.tellg() << std::endl;
        ...
        case BrtCellIsst: {  // shared string
          ...
          // out << "<c r=\"" << int_to_col(val1 + 1) << row + 1 << "\"" <<
          // cell_style(val2) << " t=\"s\">" << std::endl; out << "<v>" << val3
          // << "</v>" << std::endl; out << "</c>" << std::endl;
        ...
        case BrtCellBool: {  // bool
          ...
          // out << "<c r=\"" << int_to_col(val1 + 1) << row + 1<< "\"" <<
          // cell_style(val2) << " t=\"b\">" << std::endl; out << "<v>" <<
          // (int32_t)val3 << "</v>" << std::endl; out << "</c>" << std::endl;
        ...
        case BrtCellRk: {  // integer?
          ...
          // out << "<c r=\"" << int_to_col(val1 + 1) << row + 1<< "\"" <<
          // cell_style(val2) << ">" << std::endl; out << "<v>" <<
          // std::setprecision(16) << RkNumber(val3) << "</v>" << std::endl; out
          // << "</c>" << std::endl;
        ...
        case BrtCellReal: {
          ...
          // out << "<c r=\"" << int_to_col(val1 + 1) << row + 1 << "\"" <<
          // cell_style(val2) << ">" << std::endl; out << "<v>" <<
          // std::setprecision(16) << dbl << "</v>" << std::endl; // <<
          // std::fixed out << "</c>" << std::endl;
        ...
        case BrtCellBlank: {
          ...
          // out << "<c r=\"" << int_to_col(blank[0] + 1) << row + 1 << "\"" <<
          // cell_style(blank[1]) << "/>" << std::endl;
        ...
        case BrtCellError: {  // t="e" & <v>#NUM!</v>
          ...
          // out << "<c r=\"" << int_to_col(val1 + 1) << row + 1<< "\"" <<
          // cell_style(val2) << " t=\"e\">" << std::endl; out << "<v>" <<
          // BErr(bin, swapit) << "</v>" << std::endl; out << "</c>" <<
          // std::endl;
        ...
        case BrtCellMeta: {
          if (debug)
            Rcpp::Rcout << "BrtCellMeta: " << bin.tellg() << std::endl;
          uint32_t icmb = 0;
          icmb = readbin(icmb, bin, swapit);

          if (debug)
            Rcpp::Rcout << "cell contains unhandled cell metadata entry" << icmb
                        << std::endl;
        ...
        case BrtFmlaBool: {
          ...
          // GrbitFmlaFields *fields = (GrbitFmlaFields *)&grbitFlags;
          std::string fml =
              CellParsedFormula(bin, swapit, debug, 0, row, is_shared_formula,
                                has_revision_record);
          ...
          if ((fml.empty() || sfml != shared_cells.end()) &&
              sfml1 != shared_cells.end()) {
            column.f_t = fml_type;
          ...
          column.c_t = "b";
          column.c_r = a1;
          if (fields->fAlwaysCalc)
            column.f_ca = std::to_string(fields->fAlwaysCalc);
          colvec.push_back(column);
        ...
        case BrtArrFmla: {
          if (debug)
            Rcpp::Rcout << "BrtArrFmla: " << bin.tellg() << std::endl;
          int32_t is_shared_formula = false;
          ...
          std::string fml =
              CellParsedFormula(bin, swapit, debug, col, row, is_shared_formula,
                                has_revision_record);
          if (debug)
            Rcpp::Rcout << "BrtArrFmla: " << fml << std::endl;
        ...
        case BrtShrFmla: {
          if (debug)
            Rcpp::Rcout << "BrtShrFmla: " << bin.tellg() << std::endl;
          int32_t is_shared_formula = false;
          ...
          std::vector<std::string> cells =
              dims_to_cells(rwFirst, rwLast, colFirst, colLast);
          ...
          colvec[last].f = fml;
          colvec[last].f_t =
              fml_type;  // not sure how many cells share array type
          colvec[last].f_ref = ref;
        ...
        case BrtMergeCell: {
          ...
          if (debug)
            Rprintf("MergeCell: %d %d %d %d\n", rwFirst, rwLast, colFirst,
                    colLast);

          out << "<mergeCell ref=\"" << int_to_col(colFirst) << rwFirst << ":"
              << int_to_col(colLast) << rwLast << "\" />" << std::endl;
        ...
        case BrtMargins: {
          ...
          double xnumLeft = 0, xnumRight = 0, xnumTop = 0, xnumBottom = 0,
                 xnumHeader = 0, xnumFooter = 0;
          ...
          if (xnumLeft < 0 || xnumLeft > 49)
            Rcpp::stop("xnumLeft size out of bounds");
          if (xnumRight < 0 || xnumRight > 49)
            Rcpp::stop("xnumRight size out of bounds");
          if (xnumTop < 0 || xnumTop > 49)
            Rcpp::stop("xnumTop size out of bounds");
          if (xnumBottom < 0 || xnumBottom > 49)
            Rcpp::stop("xnumBottom size out of bounds");
          if (xnumHeader < 0 || xnumHeader > 49)
            Rcpp::stop("xnumHeader size out of bounds");
          if (xnumFooter < 0 || xnumFooter > 49)
            Rcpp::stop("xnumFooter size out of bounds");

          out << "<pageMargins" << " left=\"" << xnumLeft << "\"" << " right=\""
              << xnumRight << "\"" << " top=\"" << xnumTop << "\""
              << " bottom=\"" << xnumBottom << "\"" << " header=\""
              << xnumHeader << "\"" << " footer=\"" << xnumFooter << "\""
              << " />" << std::endl;
        ...
        case BrtDrawing: {
          if (debug)
            Rcpp::Rcout << "BrtDrawing" << std::endl;
        ...
        case BrtLegacyDrawing: {
          if (debug)
            Rcpp::Rcout << "BrtLegacyDrawing" << std::endl;
          std::string stRelId = XLNullableWideString(bin, swapit);
        ...
        case BrtLegacyDrawingHF: {
          if (debug)
            Rcpp::Rcout << "BrtLegacyDrawingHF" << std::endl;
          std::string stRelId = XLNullableWideString(bin, swapit);
        ...
        case BrtHLink: {
          ...
          std::string hlink = "<hyperlink display=\"" + escape_xml(display) +
                              "\" r:id=\"" + relId + "\" location=\"" +
                              escape_xml(location) + "\" ref=\"" + ref +
        ...
        case BrtBeginAFilter: {
          if (debug)
            Rcpp::Rcout << "BrtBeginAFilter" << std::endl;
          std::vector<int32_t> rfx = UncheckedRfX(bin, swapit);
        ...
        case BrtBeginFilterColumn: {
          if (debug)
            Rcpp::Rcout << "BrtBeginFilterColumn" << std::endl;
        ...
        case BrtBeginFilters: {
          if (debug)
            Rcpp::Rcout << "BrtBeginFilters" << std::endl;
          // bin.seekg(size, bin.cur);
        ...
        case BrtEndFilters: {
          if (debug)
            Rcpp::Rcout << "BrtEndFilters" << std::endl;
          out << "</filters>" << std::endl;
          bin.seekg(size, bin.cur);
          break;
        }
        ...
        case BrtColorFilter: {
          if (debug)
            Rcpp::Rcout << "BrtColorFilter" << std::endl;
          uint32_t dxfid = 0, fCellColor = 0;
          dxfid = readbin(dxfid, bin, swapit);
          fCellColor = readbin(fCellColor, bin, swapit);
        ...
        case BrtBeginUserShViews: {
          if (debug)
            Rcpp::Rcout << "BrtBeginUserXXViews" << std::endl;
        ...
        case BrtBeginUserCsView: {
          if (debug)
            Rcpp::Rcout << "BrtBeginUserCsView" << std::endl;

          int32_t guid0 = 0, guid1 = 0, guid2 = 0, guid3 = 0, iTabId = 0,
                  dwScale = 0, flags = 0;
        ...
        case BrtBeginUserShView: {
          if (debug)
            Rcpp::Rcout << "BrtBeginUserShView" << std::endl;

          int32_t guid0 = 0, guid1 = 0, guid2 = 0, guid3 = 0, iTabId = 0,
                  dwScale = 0, icv = 0, flags = 0;
          ...
          if (rfx[0] > 1 && rfx[2] > 0)
            out << " topLeftCell=\"" << int_to_col(rfx[2] + 1)
                << std::to_string(rfx[0] + 1) << "\"";
        ...
        case BrtEndUserShView: {
          if (debug)
            Rcpp::Rcout << "BrtEndUserXXView" << std::endl;
          out << "</customSheetView>" << std::endl;
          break;
        }
        ...
        case BrtEndUserShViews: {
          if (debug)
            Rcpp::Rcout << "BrtEndUserXXViews" << std::endl;
          out << "</customSheetViews>" << std::endl;
          break;
        }
        ...
        case BrtDynamicFilter: {
          if (debug)
            Rcpp::Rcout << "BrtDynamicFilter" << std::endl;
          Rcpp::warning("Dynamic Filter found. This is not handled.");
          bin.seekg(size, bin.cur);
          break;
        }
        ...
        case BrtIconFilter14: {
          if (debug)
            Rcpp::Rcout << "<BrtIconFilter>" << std::endl;
          uint32_t iIconSet = 0, iIcon = 0;
          iIconSet = readbin(iIconSet, bin, swapit);
          iIcon = readbin(iIcon, bin, swapit);
        ...
        case BrtEndFilterColumn: {
          if (debug)
            Rcpp::Rcout << "BrtEndFilterColumn" << std::endl;
          out << "</filterColumn>" << std::endl;
          bin.seekg(size, bin.cur);
          break;
        }
        ...
        case BrtBeginListParts: {
          if (debug)
            Rcpp::Rcout << "<tableParts>" << std::endl;
          uint32_t cParts = 0;
        ...
        case BrtBeginOleObjects: {
          if (debug)
            Rcpp::Rcout << "<oleObjects>" << std::endl;
        ...
        case BrtOleObject: {
          ...
          int32_t sharedFormula = false;
          if (fLinked)
            std::string link = CellParsedFormula(
                bin, swapit, debug, 0, 0, sharedFormula, has_revision_record);

          std::string stRelID = XLNullableWideString(bin, swapit);
          out << "<oleObject progId=\"" << strProgID << "\" shapeId=\""
              << shapeId << "\" r:id=\"" << stRelID << "\" />" << std::endl;
          break;
        }
        ...
        case BrtBeginDVals: {
          if (debug)
            Rcpp::Rcout << "BrtBeginDVals" << std::endl;
        ...
        case BrtDVal: {
          if (debug)
            Rcpp::Rcout << "BrtDVal" << std::endl;

          uint32_t flags = 0;
          std::vector<int32_t> sqrfx;
          std::string strErrorTitle = "", strError = "", strPromptTitle = "",
                      strPrompt = "";
          ...
          int32_t sharedFormula = false;
          std::string formula1 = CellParsedFormula(
              bin, swapit, debug, 0, 0, sharedFormula, has_revision_record);
          std::string formula2 = CellParsedFormula(
              bin, swapit, debug, 0, 0, sharedFormula, has_revision_record);
          ...
          out << "<dataValidation " << "type=\"" << valType(fields->valType)
              ...
              << "sqref=\"" << sqref << "\" >" << "<formula1>" << formula1
              << "</formula1>" << "<formula2>" << formula2 << "</formula2>"
        ...
        case BrtEndDVals: {
          if (debug)
            Rcpp::Rcout << "BrtEndDVals" << std::endl;
          out << "</dataValidations>" << std::endl;
          bin.seekg(size, bin.cur);
          break;
        }
        ...
        case BrtBeginDVals14: {
          if (debug)
            Rcpp::Rcout << "BrtBeginDVals14" << std::endl;
          ...
          out << "<x14:dataValidations count=\"" << std::to_string(idvMac)
              << "\" "
                 "xmlns:xm=\"http://schemas.microsoft.com/office/excel/2006/"
                 "main\""
                 ">"
              << std::endl;
        ...
        case BrtDVal14: {
          ...
          if (fields_frt->fSqref) {  // must be 1
            ...
            for (uint32_t csqrf = 0; csqrf < csqref; ++csqrf) {
              ...
              std::string lref = int_to_col(rfx_csqrfx[3] + 1) +
                                 std::to_string(rfx_csqrfx[1] + 1);
              std::string rref = int_to_col(rfx_csqrfx[4] + 1) +
                                 std::to_string(rfx_csqrfx[2] + 1);
          ...
          if (fields_frt->fFormula) {  // 0 or 1
            ...
            for (uint32_t cfml = 0; cfml < cformula; ++cfml) {
              ...
              int32_t sharedFormula = false;
              std::string frt_fml = FRTParsedFormula(
                  bin, swapit, debug, 0, 0, sharedFormula, has_revision_record);
          ...
          out << "<x14:dataValidation " << "type=\"" << valType(fields->valType)
              << "\" " << "operator=\"" << typOperator(fields->typOperator)
              << "\" " << "allowBlank=\"" << (bool)fields->fAllowBlank << "\" "
              << "showInputMessage=\"" << (bool)fields->fShowInputMsg << "\" "
              << "showErrorMessage=\"" << (bool)fields->fShowErrorMsg << "\" "
          ...
          for (size_t len = 0; len < frt_fmls.size(); ++len) {
            out << "<x14:formula" << std::to_string(len + 1) << ">" << "<xm:f>"
                << frt_fmls[len] << "</xm:f>" << "</x14:formula"
                << std::to_string(len + 1) << ">";
          }
          for (size_t len = 0; len < frt_fmls.size(); ++len) {
            out << "<xm:sqref>" << frt_sqrefs[len] << "</xm:sqref>";
          }
        ...
        case BrtEndDVals14: {
          if (debug)
            Rcpp::Rcout << "BrtEndDVals14" << std::endl;
        ...
        case BrtEndDCon: {
          if (debug)
            Rcpp::Rcout << "BrtBegin/EndDCon" << std::endl;
          bin.seekg(size, bin.cur);
          break;
        }
        ...
        case BrtBeginConditionalFormatting: {
          if (debug)
            Rcpp::Rcout << "<conditionalFormatting>" << std::endl;
        ...
        case BrtBeginCFRule: {
          ...
          uint16_t flags = 0;
          uint32_t iType = 0, iTemplate = 0, dxfId = 0, iPri = 0, iParam = 0,
        ...
        case BrtEndCFRule14: {
          if (debug)
            Rcpp::Rcout << "BrtBegin/EndDCon 14" << std::endl;
          bin.seekg(size, bin.cur);
          break;
        }

        // dxf styles for conditional formatting can be (or is always?) embedded
        // in the worksheet the xlsx conversion creates <x14:dxf/>
        case BrtBeginDXFs:
        case BrtDXF:
        case BrtEndDXFs: {
          if (debug)
            Rcpp::Rcout << "BrtBegin/EndDXFs in worksheet (?)" << std::endl;
          bin.seekg(size, bin.cur);
          break;
        }
        ...
        case BrtEndConditionalFormattings: {
          // UNHANDLED
          if (debug)
            Rcpp::Rcout << "</conditionalFormattings>" << std::endl;
          bin.seekg(size, bin.cur);
          break;
        }
        ...
        case BrtBrk: {
          if (debug)
            Rcpp::Rcout << "<BrtBrk>" << std::endl;

          uint32_t unRwCol = 0, unColRwStrt = 0, unColRwEnd = 0, fMan = 0,
                   fPivot = 0;
        ...
        case BrtEndRwBrk: {
          if (debug)
            Rcpp::Rcout << "<BrtEnd..Brk>" << std::endl;
        ...
        case BrtFRTBegin: {
          if (debug)
            Rcpp::Rcout << "BrtFRTBegin" << std::endl;
        ...
        case BrtEndCellIgnoreECs: {
          if (debug)
            Rcpp::Rcout << "BrtBeginCell" << std::endl;
          // Rcpp::warning("Worksheet contains unhandled data validation.");
          bin.seekg(size, bin.cur);
          break;
        }
        ...
        case BrtEndSortCond: {
          if (debug)
            Rcpp::warning("Worksheet contains unhandled sorting.");
          bin.seekg(size, bin.cur);
          break;
        }
        ...
        case BrtValueMeta: {
          if (debug)
            Rcpp::Rcout << "BrtValueMeta" << std::endl;
          int32_t ivmb = 0;
        ...
        case BrtCsProtection: {
          if (debug)
            Rcpp::Rcout << "protection" << std::endl;
          // uint16_t protpwd = 0;
          // uint32_t fLocked = 0, fObjects = 0;
          // unhandled
          if (debug)
            Rcpp::warning("Worksheet contains unhandled protection.");
          bin.seekg(size, bin.cur);
          break;
        }
