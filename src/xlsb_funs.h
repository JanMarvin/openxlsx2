...
template <typename T>
typename std::enable_if<std::is_same<T, float>::value, float>::type swap_endian(
    T t) {
  union v {
    float f;
    uint32_t i32;
...
std::string to_utf8(const std::u16string& u16str) {
  std::string utf8str;
  utf8str.reserve(u16str.length() *
                  3);  // Reserve enough space for UTF-8 characters
  ...
  for (std::size_t i = 0; i < u16str.length(); ++i) {
    ...
    if (networkOrderChar <= 0x7F) {
      ...
      if (networkOrderChar >= 0xD800 && networkOrderChar <= 0xDBFF &&
        ...
        int32_t code_point = ((high_surrogate - 0xD800) << 10) +
        ...
        utf8str.push_back(
            static_cast<char>(0x80 | ((networkOrderChar >> 6) & 0x3F)));
        utf8str.push_back(static_cast<char>(0x80 | (networkOrderChar & 0x3F)));
      }
    }
  }
...
std::string read_xlwidestring(std::string& mystring, std::istream& sas) {
  ...
  if (str.size() / 2 != size)
    Rcpp::warning("String size unexpected");
...
int32_t RECORD_SIZE(std::istream& sas, bool swapit) {
  ...
  if (sar1 & 0x80)
    sar2 = readbin(sar2, sas, swapit);
  if (sar2 & 0x80)
    sar3 = readbin(sar3, sas, swapit);
  if (sar3 & 0x80)
    sar4 = readbin(sar4, sas, swapit);

  // Rcpp::Rcout << sar1 << ": " << sar2 << ": " << sar3 << ": " << sar4 <<
  // std::endl;

  if (sar2 != 0 && sar3 != 0 && sar4 != 0) {
    int32_t recordType = ((sar4 & 0x7F) << 7) | ((sar3 & 0x7F) << 7) |
                         ((sar2 & 0x7F) << 7) | (sar1 & 0x7F);
    return recordType;
  }

  if (sar2 != 0 && sar3 != 0 && sar4 == 0) {
    int32_t recordType =
        ((sar3 & 0x7F) << 7) | ((sar2 & 0x7F) << 7) | (sar1 & 0x7F);
    return recordType;
  }
...
std::vector<std::pair<int, int>> StrRun(std::istream& sas,
                                        uint32_t dwSizeStrRun,
                                        bool swapit) {
...
std::string utf8_substr(const std::string& str, int32_t start, int32_t length) {
  ...
  while (char_pos < static_cast<size_t>(start + length) &&
         byte_pos < str.size()) {
    if ((str[byte_pos] & 0x80) == 0) {
      // Single-byte character (ASCII)
      byte_pos += 1;
...
std::string to_rich_text(const std::string& str,
                         const std::vector<std::pair<int, int>>& str_runs) {
  std::string result;
  ...
  for (size_t str_run = 0; str_run < str_runs.size(); ++str_run) {
    if (str_run == 0 && str_runs[0].first > 0) {
      ...
      result += "<r><FONT_" + std::to_string(str_runs[str_run].second) + "/>" +
                wrap_xml(escape_xml(part)) + "</r>";
    }
...
std::string RichStr(std::istream& sas, bool swapit) {
  ...
  if (A) {  //  || unk
    ...
    if (dwSizeStrRun > 0x7FFF)
      Rcpp::stop("dwSizeStrRun to large");
  ...
  if (B) {
    ...
    if (dwPhoneticRun > 0x7FFF)
      Rcpp::stop("dwPhoneticRun to large");
...
void ProductVersion(std::istream& sas, bool swapit, bool debug) {
  ...
  // fields->product, fields->reserved);
}
...
std::vector<std::string> dims_to_cells(int32_t firstRow,
                                       int32_t lastRow,
                                       int32_t firstCol,
                                       int32_t lastCol) {
...
std::vector<int32_t> brtColor(std::istream& sas, bool swapit) {
  uint8_t AB = 0, xColorType = 0, index = 0, bRed = 0, bGreen = 0, bBlue = 0,
          bAlpha = 0;
...
std::string to_argb(int32_t a, int32_t r, int32_t g, int32_t b) {
  ...
  out << std::uppercase << std::hex << std::setw(2) << std::setfill('0')
      << (int32_t)a << std::setw(2) << std::setfill('0') << (int32_t)r
      << std::setw(2) << std::setfill('0') << (int32_t)g << std::setw(2)
      << std::setfill('0') << (int32_t)b;
  return out.str();
}
...
std::string guid_str(const std::vector<int32_t>& guid_ints) {
  ...
  guidStream << std::setw(2) << ((guid_ints[0] >> 24) & 0xFF) << std::setw(2)
             << ((guid_ints[0] >> 16) & 0xFF) << std::setw(2)
             << ((guid_ints[0] >> 8) & 0xFF) << std::setw(2)
             << (guid_ints[0] & 0xFF) << "-";

  guidStream << std::setw(2) << ((guid_ints[1] >> 8) & 0xFF) << std::setw(2)
             << (guid_ints[1] & 0xFF) << "-";

  guidStream << std::setw(2) << ((guid_ints[1] >> 24) & 0xFF) << std::setw(2)
             << ((guid_ints[1] >> 16) & 0xFF) << "-";

  guidStream << std::setw(2) << (guid_ints[2] & 0xFF) << std::setw(2)
             << ((guid_ints[2] >> 8) & 0xFF) << "-" << std::setw(2)
             << ((guid_ints[2] >> 16) & 0xFF) << std::setw(2)
             << ((guid_ints[2] >> 24) & 0xFF) << std::setw(2)
             << (guid_ints[3] & 0xFF) << std::setw(2)
             << ((guid_ints[3] >> 8) & 0xFF) << std::setw(2)
             << ((guid_ints[3] >> 16) & 0xFF) << std::setw(2)
             << ((guid_ints[3] >> 24) & 0xFF);
...
std::string array_elements(const std::vector<std::string>& elements,
                           int32_t n,
                           int32_t k) {
  std::stringstream ss;
  ss << "{";
...
std::string parseRPN(const std::string& expression) {
  ...
  while (std::getline(iss, line)) {
    ...
    //     // Rcpp::Rcout << "Formula stacksize is: " << formulaStack.size() <<
    //     std::endl; std::string operand2 = formulaStack.top();
    //     formulaStack.pop();
    //     std::string operand1 = formulaStack.top();
    //     formulaStack.pop();
...
std::string rgce(std::string fml_out,
                 std::istream& sas,
                 bool swapit,
                 bool debug,
                 int32_t col,
  ...
  while (sas.tellg() < pos) {
    ...
    if (controlbit != 0)
      Rcpp::warning("controlbit unexpectedly not 0");
    ...
    switch (val1) {
      ...
      case PtgAttr: {
        ...
        if (debug)
          Rcpp::Rcout << "PtgAttr: " << std::hex << (int)val1 << ": "
                      << (int)val2 << std::dec << std::endl;
        ...
        switch (val2) {
          case PtgList: {
            ...
            if (debug)
              Rcpp::Rcout << "PtgList " << sas.tellg() << std::endl;
            ...
            paddedStr << std::setw(12) << std::setfill('0')
                      << listIndex;  // << ixti;
            ...
            bool need_bracket =
                fields->columns > 0 ||
                (fields->columns == 0 && (fields->rowType == dataheaders ||
            ...
            if (!no_row_type && fields->rowType) {
              if (need_bracket)
                fml_out += "[";
              if (fields->rowType == data)
                fml_out += "";
              if (fields->rowType == all)
                fml_out += "#All";
              if (fields->rowType == headers)
                fml_out += "#Headers";
              if (fields->rowType == data2)
                fml_out += "#Data";
              if (fields->rowType == dataheaders)
                fml_out += "#Headers],[#Data";
              if (fields->rowType == totals)
                fml_out += "#Totals";
              if (fields->rowType == datatotals)
                fml_out += "#Data],[#Totals";
              if (fields->rowType == current)
                fml_out += "#This Row";
              if (need_bracket)
                fml_out += "]";
              if (fields->columns > 0)
            ...
            if (!(fields->invalid == 1 || fields->nonresident == 1 ||
                  fields->columns == 0)) {
              // Rcpp::Rcout << "colFirst" << std::endl;
              if (fields->columns > 1 || fields->rowType > data)
                fml_out += "[";
            ...
            if ((colFirst < colLast) &&
                !(fields->invalid == 1 || fields->nonresident == 1 ||
                  fields->columns == 0)) {
            ...
            if (debug)
              Rprintf("PtgList: %d, %d, %d, %d\n", ixti, listIndex, colFirst,
                      colLast);

            // if (debug)
            // Rcpp::warning("formulas with table references are not
            // implemented.");
          ...
          case PtgAttrChoose: {
            if (debug)
              Rcpp::Rcout << "PtgAttrChoose" << std::endl;
          ...
          case PtgAttrSum: {
            ...
            // Rcpp::Rcout << unused << std::endl;
            fml_out +=
                "SUM(%s)";  // maybe attr because it is a single cell function?
            fml_out += "\n";
            break;
          }
      ...
      case PtgStr: {
        ...
        if (esc_quote == "\n")
          ...
          fml_out += "\"" + escape_xml(escape_quote(esc_quote)) + "\"";
        fml_out += "\n";
      ...
      case PtgArray3: {
        ...
        // NOTE: maybe it's better to push the formula to a string vector and
        // find and replace @array@ with array information
        fml_out += "@array@";
        fml_out += "\n";
      ...
      case PtgMemArea3: {
        if (debug)
          Rcpp::Rcout << "PtgMemArea" << std::endl;
      ...
      case PtgRefErr3d3: {
        if (debug)
          Rcpp::Rcout << "PtgRefErr3d" << std::endl;
      ...
      case PtgAreaErr3: {
        if (debug)
          Rcpp::Rcout << "PtgAreaErr" << std::endl;
      ...
      case PtgAreaErr3d3: {
        if (debug)
          Rcpp::Rcout << "PtgAreaErr3d" << std::endl;
      ...
      case PtgFuncVar3: {
        if (debug)
          Rcpp::Rcout << "PtgFuncVar" << std::endl;
        ...
        fml_out += "(";
        if (cparams)
          fml_out += "%s";  // RAND() needs no argument
      ...
      case PtgMemFunc3: {
        if (debug)
          Rcpp::Rcout << "PtgMemFunc" << std::endl;

        uint16_t cce = 0;
        cce = readbin(
            cce, sas,
            swapit);  // count of bytes in the binary reference expression
        break;
      }
      ...
      case PtgMemErr3: {
        ...
        if (debug)
          Rcpp::Rcout << "PtgMemErr: " << err_str << std::endl;
      ...
      case PtgMemNoMem3: {
        if (debug)
          Rcpp::Rcout << "PtgMemNoMem" << std::endl;
...
std::string rgcb(std::string fml_out,
                 std::istream& sas,
                 bool swapit,
                 bool debug,
                 int32_t col,
  ...
  for (size_t cntr = 0; cntr < ptgextra.size(); ++cntr) {
    ...
    switch (val1) {
      case PtgExtraCol: {  // PtgExtraCol

        // need_ptg_extra_col = true;
        if (debug)
          Rcpp::Rcout << "PtgExtraCol" << std::endl;
        int32_t ptg_col = UncheckedCol(sas, swapit);
        if (debug)
          Rcpp::Rcout << "cb PtgExp: " << int_to_col(ptg_col + 1) << std::endl;
      ...
      case PtgExtraArray: {
        if (debug)
          Rcpp::Rcout << "PtgExtraArray" << std::endl;
        // PtgExtraArray
        ...
        for (int32_t rr = 0; rr < rows; ++rr) {
          for (int32_t cc = 0; cc < cols; ++cc) {
            ...
            if (debug)
              Rcpp::Rcout << (int32_t)reserved << std::endl;

            // SerBool
            if (reserved == 0x02) {
              if (debug)
                Rcpp::Rcout << "SerBool" << std::endl;
              uint8_t f = 0;
              f = readbin(f, sas, swapit);

              if (debug)
                Rcpp::Rcout << (int32_t)f << std::endl;
        ...
        if (fi != std::string::npos) {
          if (debug)
            Rcpp::Rcout << "replacing @array@" << std::endl;
          fml_out.replace(fi, 7, array);
        } else {
          if (debug)
            Rcpp::Rcout << "no  @array@" << fml_out << std::endl;
          fml_out += array;
          fml_out += "\n";
        }
      ...
      case PtgExtraMem: {
        ...
        if (debug)
          Rcpp::Rcout << "PtgExtraMem: " << (int32_t)val1 << std::endl;
      ...
      case RevNameTabid: {
        // Rcpp::stop("Skip");
        if (debug)
          Rcpp::Rcout << "RevNameTabid: " << (int32_t)val1 << std::endl;
        sas.seekg(pos, sas.beg);
        break;
      }
      ...
      case RevName: {
        // Rcpp::stop("Skip");
        if (debug)
          Rcpp::Rcout << "RevName: " << (int32_t)val1 << std::endl;
        sas.seekg(pos, sas.beg);
        break;
      }
      ...
      case PtgExtraList: {
        // Rcpp::stop("Skip");
        if (debug)
          Rcpp::Rcout << "PtgExtraList: " << (int32_t)val1 << std::endl;
        sas.seekg(pos, sas.beg);
        break;
      }
...
std::string CellParsedFormula(std::istream& sas,
                              bool swapit,
                              bool debug,
                              int32_t col,
                              int32_t row,
  ...
  fml_out = rgce(fml_out, sas, swapit, debug, col, row, sharedFml,
                 has_revision_record, pos, ptgextra);

  cb =
      readbin(cb, sas, swapit);  // is there a control bit, even if CB is empty?
  ...
  fml_out = rgcb(fml_out, sas, swapit, debug, col, row, sharedFml,
                 has_revision_record, pos, ptgextra);
...
std::string FRTParsedFormula(std::istream& sas,
                             bool swapit,
                             bool debug,
                             int32_t col,
                             int32_t row,
