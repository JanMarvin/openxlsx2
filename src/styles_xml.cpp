...
bool has_it(Rcpp::DataFrame df_xf,
...
Rcpp::DataFrame read_xf(XPtrXML xml_doc_xf) {
  ...
  std::set<std::string> nams{
      "numFmtId", "fontId", "fillId", "borderId", "xfId", "applyNumberFormat",
      ...
      "relativeIndent", "shrinkToFit", "textRotation", "vertical", "wrapText",
      // child extLst
      "extLst",
      // child protection
      "hidden", "locked"};
  ...
  for (auto xml_xf : xml_doc_xf->children("xf")) {
    ...
    for (auto cld : xml_xf.children()) {
      ...
      // check known names
      if (cld_name == "alignment" || cld_name == "extLst" ||
          cld_name == "protection") {
        if (cld_name == "extLst") {
          ...
          Rcpp::as<Rcpp::CharacterVector>(df[mtc])[itr] =
              Rcpp::String(oss.str());
        } else {
...
Rcpp::CharacterVector write_xf(Rcpp::DataFrame df_xf) {
  ...
  std::set<std::string> xf_nams = {"numFmtId",
                                   "fontId",
                                   "fillId",
                                   "borderId",
                                   "xfId",
  ...
  std::set<std::string> xf_nams_alignment = {
      "horizontal",   "indent",         "justifyLastLine",
    ...
    for (R_xlen_t j = 0; j < k; ++j) {
      ...
      if (has_extLst && is_extLst) {
        ...
        if (cv_s[0] != "") {
          ...
          pugi::xml_parse_result tempResult =
              tempDoc.load_string(val_strl.c_str());
          if (tempResult) {
...
Rcpp::DataFrame read_font(XPtrXML xml_doc_font) {
  ...
  // openxml 2.8.1
  std::set<std::string> nams{"b", "charset", "color", "condense", "extend",
                             "family", "i", "name", "outline",
                             // TODO might contain child <localName ...>
                             "scheme", "shadow", "strike", "sz", "u",
                             "vertAlign"};
    ...
    for (auto j = 0; j < df_font.ncol(); ++j) {
      ...
      if (cv_s[0] != "") {
        ...
        pugi::xml_parse_result result =
            font_node.load_string(font_i.c_str(), pugi_parse_flags);
        if (!result)
...
Rcpp::DataFrame read_border(XPtrXML xml_doc_border) {
  ...
  std::copy(nam_chlds.begin(), nam_chlds.end(),
            all_names.begin() + static_cast<R_xlen_t>(nam_attrs.size()));
    ...
    for (R_xlen_t j = 0; j < k; ++j) {
      ...
      if (nam_chlds.count(attr_j) != 0) {
        ...
        if (cv_s[0] != "") {
          ...
          pugi::xml_parse_result result =
              border_node.load_string(font_i.c_str(), pugi_parse_flags);
          if (!result)
            Rcpp::stop("loading border node fail: %s", cv_s);
    ...
    for (auto j = 0; j < df_fill.ncol(); ++j) {
      ...
      if (cv_s[0] != "") {
        ...
        pugi::xml_parse_result result =
            font_node.load_string(font_i.c_str(), pugi_parse_flags);
        if (!result)
          Rcpp::stop("loading fill node fail: %s", cv_s);
...
Rcpp::DataFrame read_cellStyle(XPtrXML xml_doc_cellStyle) {
  ...
  R_xlen_t nn =
      std::distance(xml_doc_cellStyle->begin(), xml_doc_cellStyle->end());
  R_xlen_t kk = static_cast<R_xlen_t>(nams.size());
  uint32_t pugi_format_flags = pugi::format_raw | pugi::format_no_escapes;
      ...
      if (nam_chlds.count(attr_j) != 0) {
        ...
        if (cv_s[0] != "") {
          ...
          pugi::xml_parse_result result =
              border_node.load_string(font_i.c_str(), pugi_parse_flags);
          if (!result)
            Rcpp::stop("loading cellStyle node fail: %s", cv_s);
...
Rcpp::DataFrame read_tableStyle(XPtrXML xml_doc_tableStyle) {
  ...
  R_xlen_t nn =
      std::distance(xml_doc_tableStyle->begin(), xml_doc_tableStyle->end());
  R_xlen_t kk = static_cast<R_xlen_t>(nams.size());
  uint32_t pugi_format_flags = pugi::format_raw | pugi::format_no_escapes;
    ...
    for (R_xlen_t j = 0; j < k; ++j) {
      ...
      if (nam_chlds.count(attr_j) != 0) {
        ...
        if (cv_s[0] != "") {
          ...
          pugi::xml_parse_result result =
              tableStyleElement.load_string(font_i.c_str(), pugi_parse_flags);
          if (!result)
            Rcpp::stop("loading df_tablestyle node fail: %s", cv_s);
...
Rcpp::DataFrame read_dxf(XPtrXML xml_doc_dxf) {
  ...
  // openxml 2.8.1
  std::set<std::string> nams{"alignment", "border", "extLst",    "fill",
                             "font",      "numFmt", "protection"};
    ...
    for (auto j = 0; j < df_dxf.ncol(); ++j) {
      ...
      if (cv_s[0] != "") {
        ...
        pugi::xml_parse_result result =
            font_node.load_string(font_i.c_str(), pugi_parse_flags);
        if (!result)
          Rcpp::stop("loading dxf node fail: %s", cv_s);
    ...
    for (R_xlen_t j = 0; j < k; ++j) {
      ...
      if (cv_s[0] != "") {
        ...
        pugi::xml_parse_result result =
            font_node.load_string(font_i.c_str(), pugi_parse_flags);
        if (!result)
          Rcpp::stop("loading color node fail: %s", cv_s);
