...
wb$add_conditional_formatting("Duplicates", cols = 1, rows = 1:10, type = "duplicatedValues")
```
...
wb$add_conditional_formatting("notcontainsText", cols = 1,
                     rows = 1:10, type = "notContainsText", rule = "A")
```
...
```{r}
wb$add_conditional_formatting("colorScale",
                     cols = seq_along(df), rows = 1:nrow(df),
                     style = c("black", "white"),
                     rule = c(0, 255),
                     type = "colorScale"
)
wb_set_col_widths(wb, "colorScale", cols = seq_along(df), widths = 1.07)
...
wb$add_conditional_formatting("between", cols = 1, rows = 1:11, type = "between", rule = 2)
