...
Coming from `openxlsx` you might know about `read.xlsx()`, `readWorkbook()` and `loadWorkbook()` multiple functions that do many different things, but mostly the same. In `openxlsx2` we tried our best to reduce the complexity under the hood and for the user as well. In `openxlsx2` they are replaced with `read_xlsx()`, `wb_read()` and `wb_load()` and they all share the same underlying function `wb_to_df()`.
