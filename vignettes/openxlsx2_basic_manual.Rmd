...
Coming from `openxlsx` you might know about `read_xlsx()`, `wb_read()` and `wb_load` multiple functions that do may different things, but mostly the same. In `openxlsx2` we tried our best to reduce the complexity under the hood and for the user as well. For this example we will use example data provided by the package. You can locate it in our "inst/extdata" folder. The files are included with the package source and you can open them in any calculation software as well.

### Basic import
We begin with the `readTest.xlsx` file by telling R where to find this file on our system
```{r}
...
### colNames - first row as column name
...
### detectDates - convert cells to R dates
...
### showFormula - show formulas instead of results
...
### skipEmptyRows - remove empty rows
...
### skipEmptyCols - remove empty columns
...
### rowNames - keep rownames from input
