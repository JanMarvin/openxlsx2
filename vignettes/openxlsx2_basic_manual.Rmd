...
Coming from `openxlsx` you might know about `read_xlsx()`, `readWorkbook()` and `wb_load` multiple functions that do may different things, but mostly the same. In `openxlsx2` we tried our best to reduce the complexity under the hood and for the user as well. For this example we will use example data provided by the package. You can locate it in our "inst/extdata" folder. The files are included with the package source and you can open them in any calculation software as well.
...
### skipEmptyRows - remove emty rows
