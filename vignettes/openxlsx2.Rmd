...
wb_to_df(xlsx_file, col_names = FALSE)
```
...
wb_to_df(xlsx_file, detect_dates = FALSE)
```
...
wb_to_df(xlsx_file, show_formula = TRUE)
```
...
wb_to_df(xlsx_file, dims = "A2:C5", col_names = FALSE)
```
...
wb_to_df(xlsx_file, dims = wb_dims(rows = 2:5, cols = 1:3), col_names = FALSE)
```
...
```{r}
# read selected cols
wb_to_df(xlsx_file, cols = c("A:B", "G"))
```
...
```{r}
# read selected rows
wb_to_df(xlsx_file, rows = c(2, 4, 6))
```
...
wb_to_df(xlsx_file, convert = FALSE)
```
...
wb_to_df(xlsx_file, sheet = 1, skip_empty_rows = TRUE) |> tail()
```
...
wb_to_df(xlsx_file, skip_empty_cols = TRUE)
```
...
wb_to_df(xlsx_file, sheet = 2, dims = "C6:G9", row_names = TRUE)
```
...
wb_to_df(xlsx_file, cols = c(2, 5), types = c("Var1" = 0, "Var3" = 1))
```
...
```{r}
# start in row 5
wb_to_df(xlsx_file, start_row = 5, col_names = FALSE)
```
...
wb_dims(x = mtcars, cols = "cyl", col_names = TRUE)
# rows in `x`
wb_dims(x = mtcars)
...
wb <- wb_workbook()$
  ...
  add_fill(
    dims = wb_dims(x = mtcars, rows = 0), # only column names
...
wb <- wb_workbook()$
  add_worksheet()$
  add_data(x = mtcars, dims = wb_dims_custom(col_names = TRUE))$
  add_fill(
    dims = wb_dims_custom(rows = 1:5),
    color = wb_color("yellow")
  )$
  add_fill(
    dims = wb_dims_custom(rows = 0),
    color = wb_color("cyan2")
  )
```
