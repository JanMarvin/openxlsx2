...
wb <- wb_workbook(creator = "My name here")$
  ...
  add_col_widths( # set column width
    cols = 1:6,
...
The code above uses chaining. If you prefer piping, we provide the chained functions with the prefix `wb_` so `wb_add_worksheet()`, `wb_add_data()`, `wb_add_border()` and `wb_add_col_widths()` would be the functions to use with pipes `%>%` or `|>`.
...
wb <- wb_workbook(creator = "My name here") %>%
  ...
  wb_add_col_widths( # set column width
    cols = 1:6,
    widths = c(20, rep(10, 5))
  ) %>% # remove the value in A1
